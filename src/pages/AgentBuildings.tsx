...
import { useCondos, Condo } from "@/hooks/useCondos";
...
export default function AgentBuildings() {
  ...
  const hasActiveFilters = !!locationFilter;
  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen flex w-full bg-background">
        <AgentSidebar activeTab="buildings" onTabChange={(tab) => navigate("/admin", { state: { page: tab } })} />
        <div className="flex-1 flex flex-col overflow-auto">
          <main className="flex-1 p-8">
            <div className="space-y-6">
              <div>
                <h1 className="text-2xl font-extrabold">Buildings</h1>
                <p className="text-sm text-muted-foreground mt-0.5">View building information and access details.</p>
              </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-card border rounded-lg overflow-hidden">
              <button type="button" onClick={() => toggleViewSection("photos")} className="w-full flex items-center justify-between p-4 hover:bg-secondary/30 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <span onClick={e => { e.stopPropagation(); const url = `${window.location.origin}/building-photos/${viewing.id}`; navigator.clipboard.writeText(url); toast.success("Building photo link copied"); }} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-muted-foreground hover:bg-secondary transition-colors cursor-pointer" title="Copy building photo link">
                  ...
                  {viewing.photos && viewing.photos.length > 0 ? (
                    <div className="grid grid-cols-4 gap-3">
                      {viewing.photos.map((path, i) => (
                        <img key={path} src={`${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/room-photos/${path}`} alt={`Photo ${i + 1}`} className="h-28 w-full object-cover rounded-lg cursor-pointer hover:opacity-90 transition-opacity" onClick={() => setPhotoLightboxIndex(i)} />
                      ))}
                    </div>
                  ) : (
            ...
            <div className="bg-card border rounded-lg overflow-hidden">
              <button type="button" onClick={() => toggleViewSection("details")} className="w-full flex items-center justify-between p-4 hover:bg-secondary/30 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <span onClick={e => { e.stopPropagation(); copyBuildingDetails(viewing); }} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-muted-foreground hover:bg-secondary transition-colors cursor-pointer">
            ...
            <div className="bg-card border rounded-lg overflow-hidden">
              <button type="button" onClick={() => toggleViewSection("visitor")} className="w-full flex items-center justify-between p-4 hover:bg-secondary/30 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <span onClick={e => { e.stopPropagation(); copyVisitorInfo(viewing); }} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-muted-foreground hover:bg-secondary transition-colors cursor-pointer">
            ...
            <div className="bg-card border rounded-lg overflow-hidden">
              <button type="button" onClick={() => toggleViewSection("pedestrian")} className="w-full flex items-center justify-between p-4 hover:bg-secondary/30 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <span onClick={e => { e.stopPropagation(); copyToClipboard(formatAccessText(getAccessItems(viewing, "pedestrian"), "Pedestrian Access", true), "Pedestrian access"); }} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-muted-foreground hover:bg-secondary transition-colors cursor-pointer">
              ...
              {viewSections.pedestrian && <div className="px-4 pb-4">{renderViewAccessItems(getAccessItems(viewing, "pedestrian"), true)}</div>}
            </div>
            ...
            <div className="bg-card border rounded-lg overflow-hidden">
              <button type="button" onClick={() => toggleViewSection("carpark")} className="w-full flex items-center justify-between p-4 hover:bg-secondary/30 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <span onClick={e => { e.stopPropagation(); copyToClipboard(formatAccessText(getAccessItems(viewing, "carpark"), "Car Park Access", false), "Car park access"); }} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-muted-foreground hover:bg-secondary transition-colors cursor-pointer">
              ...
              {viewSections.carpark && <div className="px-4 pb-4">{renderViewAccessItems(getAccessItems(viewing, "carpark"), false)}</div>}
            </div>
            ...
            <div className="bg-card border rounded-lg overflow-hidden">
              <button type="button" onClick={() => toggleViewSection("motorcycle")} className="w-full flex items-center justify-between p-4 hover:bg-secondary/30 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <span onClick={e => { e.stopPropagation(); copyToClipboard(formatAccessText(getAccessItems(viewing, "motorcycle"), "Motorcycle Access", false), "Motorcycle access"); }} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-muted-foreground hover:bg-secondary transition-colors cursor-pointer">
              ...
              {viewSections.motorcycle && <div className="px-4 pb-4">{renderViewAccessItems(getAccessItems(viewing, "motorcycle"), false)}</div>}
            </div>
          </div>
          );
        })()}
